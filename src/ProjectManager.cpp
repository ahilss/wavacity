...
#ifdef __WXWASM__
  // Displays prompt before exiting the page
   event.Veto();
   return;
#endif
...
void ProjectManager::OpenFiles(WavvyProject *proj)
{
   ...
   ProjectFileManager::ShowOpenDialog(FileNames::Operation::Open,
      extraType,
      [proj](const wxArrayString& files) {
         wxArrayString selectedFiles = files;
         ...
         for (size_t ff = 0; ff < selectedFiles.size(); ff++) {
            ...
            newProj = OpenProject( newProj, fileName );
            //newProj = OpenProject( proj, fileName );
         }
      });
}
