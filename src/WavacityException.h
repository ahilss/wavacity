#ifndef __WAVACITY_EXCEPTION__
#define __WAVACITY_EXCEPTION__

/*!********************************************************************

 Audacity: A Digital Audio Editor

 @file WavacityException.h
 @brief Declare abstract class WavacityException, some often-used subclasses, and @ref GuardedCall
 ...
 */
class WavacityException /* not final */
{
public:
   WavacityException() {}
   virtual ~WavacityException() = 0;
...
protected:
   ...
   WavacityException( const WavacityException& ) = default;

   //! Don't allow moves of this class or subclasses
   // see https://bugzilla.audacityteam.org/show_bug.cgi?id=2442
   WavacityException( WavacityException&& ) = delete;

   //! Disallow assignment
   WavacityException &operator = ( const WavacityException & ) = delete;
};

//! Abstract WavacityException subclass displays a message, specified by further subclass
...
class MessageBoxException /* not final */ : public WavacityException
{
   //! Privatize the inherited function
   using WavacityException::DelayedHandlerAction;
...
{
   void operator () (WavacityException *pException) const
   {
      if ( pException )
...
template <typename R> struct SimpleGuard
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   R operator () ( WavacityException * ) const { return m_value; }
   const R m_value;
};
...
template<> struct SimpleGuard<bool>
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   bool operator () ( WavacityException * ) const { return m_value; }
...
{
   SimpleGuard() {}
   void operator () ( WavacityException * ) const {}
...
/*!
  ...
  If WavacityException is handled, then it queues up a delayed handler action for execution later in
  the event loop at idle time, on the main thread; typically this informs the user of the error.

  The default delayed handler action is simply to invoke a method of the WavacityException, but this
  ...
  takes pointer to WavacityException, which is null when some other type of exception is caught;
  return value is converted to @b R
  @tparam F3 deduced type of delayed handler function, if a nondefault argument is given;
  takes pointer to WavacityException, return value is unused
 */
template <
   typename R = void,
   ...
   typename F2 = SimpleGuard< R >, // function object
      // with signature R( WavacityException * )

   typename F3 =
      DefaultDelayedHandlerAction // Any( WavacityException * ), ignore return
>
//! Execute some code on any thread; catch any WavacityException; enqueue error report on the main thread
R GuardedCall(
