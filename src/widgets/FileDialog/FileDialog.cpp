...
void FileSelector(const wxString& title,
                      ...
                      std::function<void (const wxString&)> callback)
{
    ...
    FileDialog *fileDialog = safenew FileDialog(parent, title, defaultDir,
    ...
    fileDialog->ShowModal([callback, fileDialog] (int retCode) {
       wxString filename;
       if (retCode == wxID_OK)
       {
