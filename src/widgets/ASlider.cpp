...
void LWSlider::ShowDialog(std::function<void (bool)> callback)
{
   DoShowDialog( mParent->ClientToScreen(wxPoint( mLeft, mTop ) ), callback );
}

void LWSlider::ShowDialog(wxPoint pos, std::function<void (bool)> callback)
{
   DoShowDialog( pos, callback );
}

void LWSlider::DoShowDialog(wxPoint pos, std::function<void (bool)> callback)
{
   float oldValue = mCurrentValue;

   SliderDialog *dlg = safenew SliderDialog( NULL,
                     wxID_ANY,
                     mName,
                     pos,
   ...
   dlg->ShowModal([this, dlg, oldValue, callback] (int retCode) {
      bool changed = (retCode == wxID_OK);
...
void ASlider::ShowDialog(wxPoint pos, std::function<void (bool)> callback)
{
   mLWSlider->ShowDialog(pos, callback);
}
