...
BEGIN_EVENT_TABLE(NumericTextCtrl, wxControl)
   ...
   EVT_MENU(wxID_ANY, NumericTextCtrl::OnMenu)
END_EVENT_TABLE()
...
bool NumericTextCtrl::Layout()
{
   ...
   mBackgroundBitmap = std::make_unique<wxBitmap>();
   mBackgroundBitmap->CreateScaled(mWidth + mButtonWidth, mHeight, 24, GetContentScaleFactor());
