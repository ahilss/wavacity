...
void OnResample(const CommandContext &context)
{
   ...
   //while (true)
   {
      wxDialogWrapper *dlg = safenew wxDialogWrapper(&window, wxID_ANY, XO("Resample"));
      ShuttleGui S(dlg, eIsCreating);
      wxChoice *cb;

      const TranslatableString& rate = XO("%ld").Format(lrint(projectRate));

      TranslatableStrings rates{
         XO("8000") ,
         XO("11025") ,
         XO("16000") ,
         XO("22050") ,
      ...
      S.StartVerticalLay(true);
      {
         ...
         {
            cb = S.AddChoice(XXO("New sample rate (Hz):"),
                             rates,
                             rate);
         }
         S.EndHorizontalLay();
      ...
      dlg->ShowModal([&project, dlg, cb](int retCode) {
...
void OnTrackPan(const CommandContext &context)
{
   ...
   if (track) track->TypeSwitch( [&](WaveTrack *wt) {
      ...
      slider->ShowDialog([&project, wt, slider] (bool retval) {
         if (retval)
