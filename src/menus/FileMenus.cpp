...
void DoImport(const CommandContext &context, bool isRaw)
{
   ...
   FileNames::FileType extraType = {};

   ProjectFileManager::ShowOpenDialog(FileNames::Operation::Import,
      extraType,
      [isRaw, &project](const wxArrayString& files) {
...
void OnExportLabels(const CommandContext &context)
{
   ...
   Exporter::ExportFile(fName);
}

void OnExportMultiple(const CommandContext &context)
{
   auto &project = context.project;
   ExportMultipleDialog *em = safenew ExportMultipleDialog(&project);

   em->ShowModal([em](int retCode) {
      delete em;
   });
}
...
void OnImportLabels(const CommandContext &context)
{
   ...
   FileNames::SelectFile(FileNames::Operation::Open,
         ...
         &window,    // Parent
         [&project](const FilePath &fileName) {
     auto &trackFactory = WaveTrackFactory::Get( project );
     auto &tracks = TrackList::Get( project );
     auto &window = ProjectWindow::Get( project );
