...
void OnSetLeftSelection(const CommandContext &context)
{
   ...
   else
   {
      ...
      TimeDialog *dlg = safenew TimeDialog(&window, XO("Set Left Selection Boundary"),
         fmt, rate, selectedRegion.t0(), XO("Position"));

      dlg->ShowModal([&project, &selectedRegion, dlg](int retCode) {
         if (wxID_OK == retCode)
         {
...
void OnSetRightSelection(const CommandContext &context)
{
   ...
   else
   {
      ...
      TimeDialog *dlg = safenew TimeDialog(&window, XO("Set Right Selection Boundary"),
