...
BEGIN_EVENT_TABLE(SelectionBar, ToolBar)
   ...
   EVT_CHOICE(RateID, SelectionBar::OnRate)
...
*/
   mRateBox = safenew wxChoice(this, RateID,
                             wxDefaultPosition, wxDefaultSize);
#if wxUSE_ACCESSIBILITY
   // so that name can be set on a standard control
   ...
   mRateBox->Append(wxString::Format(wxT("%d"), (int)mRate));
   mRateBox->SetSelection(0);
...
void SelectionBar::SetRate(double rate)
{
   if (rate != mRate) {
      ...
      auto rateString = wxString::Format(wxT("%d"), (int)rate);
      int index = mRateBox->FindString(rateString);
      if (index == wxNOT_FOUND) {
        mRateBox->Append(rateString);
        index = mRateBox->GetCount() - 1;
      }
      mRateBox->SetSelection(index);
...
{
   int selection = mRateBox->GetSelection();
   auto value = mRateBox->GetString(mRateBox->GetSelection());
...
{
   wxString oldValue = mRateBox->GetString(mRateBox->GetSelection());
   mRateBox->Clear();
   for (int i = 0; i < AudioIOBase::NumStandardRates; i++) {
      mRateBox->Append(
         wxString::Format(wxT("%d"), AudioIOBase::StandardRates[i]));
   }
   int index = mRateBox->FindString(oldValue);
   if (index == wxNOT_FOUND) {
      mRateBox->Append(oldValue);
      index = mRateBox->GetCount() - 1;
   }
   mRateBox->SetSelection(index);
}
