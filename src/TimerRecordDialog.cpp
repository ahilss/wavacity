...
static double wxDateTime_to_WavacityTime(wxDateTime& dateTime)
{
...
: wxDialogWrapper(parent, -1, XO("Wavacity Timer Record"), wxDefaultPosition,
...
void TimerRecordDialog::OnTimer(wxTimerEvent& WXUNUSED(event))
{
   ...
   if (m_DateTime_Start < dateTime_UNow) {
      ...
      m_pTimeTextCtrl_Start->SetValue(wxDateTime_to_WavacityTime(m_DateTime_Start));
...
void TimerRecordDialog::OnDatePicker_End(wxDateEvent& WXUNUSED(event))
{
   ...
   if (m_DateTime_End < m_DateTime_Start) {
      ...
      m_pTimeTextCtrl_End->SetValue(wxDateTime_to_WavacityTime(m_DateTime_End));
   }
   ...
   if (updateResult != ProgressResult::Success)  {
      ...
      TimerProgressDialog
         progress(m_TimeSpan_Duration.GetMilliseconds().GetValue(),
                  XO("Wavacity Timer Record Progress"),
                  columns,
...
int TimerRecordDialog::ExecutePostRecordActions(bool bWasStopped) {
   ...
   // Once completed, we can also close Wavacity, restart the system or
   // shutdown the system.
         ...
         S.StartStatic(XO("Start Date and Time"), true);
         {
            ...
            m_pTimeTextCtrl_Start = safenew NumericTextCtrl(
               ...
               Options{}
                  .MenuEnabled(false)
                  .Format(strFormat)
                  .Value(true, wxDateTime_to_WavacityTime(m_DateTime_Start)));
         ...
         S.StartStatic(XO("End Date and Time"), true);
         {
            ...
            m_pTimeTextCtrl_End = safenew NumericTextCtrl(
               ...
               Options{}
                  .MenuEnabled(false)
                  .Format(strFormat)
                  .Value(true, wxDateTime_to_WavacityTime(m_DateTime_End)));
            S.Name(XO("End Time"))
...
{
   //v Use remaining disk -> record time calcs from WavacityProject::OnTimer to set range?
   m_DateTime_End = m_DateTime_Start + m_TimeSpan_Duration;
...
ProgressResult TimerRecordDialog::WaitForStart()
{
   ...
   TimerProgressDialog progress(waitDuration.GetMilliseconds().GetValue(),
      XO("Wavacity Timer Record - Waiting for Start"),
      columns,
...
ProgressResult TimerRecordDialog::PreActionDelay(int iActionIndex, TimerRecordCompletedActions eCompletedActions)
{
   ...
   /* i18n-hint: %s is one of "Do nothing", "Exit Wavacity", "Restart system",
   ...
   TimerProgressDialog dlgAction(tsWait.GetMilliseconds().GetValue(),
                          XO("Wavacity Timer Record - Waiting"),
                          columns,
