...
void Tags::ShowEditDialog(wxWindow *parent, const TranslatableString &title, bool force, std::function<void (bool)> callback)
{
   if (force) {
      TagsEditorDialog *dlg = safenew TagsEditorDialog(parent, title, this, mEditTitle, mEditTrackNumber);

      dlg->ShowModal([dlg, callback](int retCode) {
         dlg->Destroy();
         if (callback)
            callback(retCode == wxID_OK);
      });
   } else if (callback) {
      callback(true);
   }
...
// TODO(ahilss): enable when dialog resizing supported on wasm
#ifndef __WXWASM__
   wxRect r = GetRect();
