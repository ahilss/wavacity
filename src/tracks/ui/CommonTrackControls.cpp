...
void TrackMenuTable::OnSetName(wxCommandEvent &)
{
   ...
   if (pTrack)
   {
      ...
      Command->PromptUser( &GetProjectFrame( *proj ), [this, pTrack, proj, oldName, Command](bool bResult) {;
        if (mpData->pTrack == pTrack && &mpData->project == proj && bResult)
        {
   ...
   const wxRect &rect, wxWindow *pParent, wxPoint *, WavvyProject *pProject)
{
   ...
   InitMenuData *data = new InitMenuData { *pProject, track.get(), pParent, RefreshNone };
   ...
   pParent->PopupMenu
      (pMenu.release(), buttonRect.x + 1, buttonRect.y + buttonRect.height + 1);
