   ...
   const TranslatableString &shortUndoDescription, bool force)
{
   ...
   newTags->ShowEditDialog(&GetProjectFrame( project ), title, force, [&project, title, shortUndoDescription, newTags](bool retval) {
      auto &tags = Tags::Get( project );
...
#ifdef __WXWASM__
         mFilename = useFileName;
         mFilename.SetPath("/tmp/");
#else
...
bool Exporter::ExportTracks()
{
   ...
   if (success) {
      success = ExportFile(mActualName);
   }

   return success;
}

bool Exporter::ExportFile(const wxFileName& path)
{
#ifdef __WXWASM__
   wxFile file;

   if (file.Open(path.GetFullPath(), wxFile::read)) {
      ...
      if (file.Read(buffer, length) == length) {
        ...
        }, static_cast<const char*>(filename.mb_str(wxConvUTF8)), length, buffer);
     } else {
        return false;
     }
   } else {
