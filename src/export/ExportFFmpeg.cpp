...
public:
   ...
   bool Init(const char *shortname, WavvyProject *project, const Tags *metadata, int subformat);

   /// Codec initialization
   bool InitCodecs(WavvyProject *project);
...
bool ExportFFmpeg::Init(const char *shortname, WavvyProject *project, const Tags *metadata, int subformat)
{
...
"FFmpeg : ERROR - Can't determine format description for file \"%s\".")
   ...
   // At the moment Wavvy can export only one audio stream
...
bool ExportFFmpeg::InitCodecs(WavvyProject *project)
{
...
//FIXME The list of supported options for the selected encoder should be extracted instead of a few hardcoded
   ...
   if (mEncAudioCodecCtx->codec_id == AV_CODEC_ID_AC3)
   {
      ...
      // But, currently, Wavvy only supports SAMPLE_FMT_S16.  So, for now, look for the
...
ProgressResult ExportFFmpeg::Export(WavvyProject *project,
   std::unique_ptr<ProgressDialog> &pDialog,
