...
public:
   ...
   bool Init(const char *shortname, WavacityProject *project, const Tags *metadata, int subformat);

   /// Codec initialization
   bool InitCodecs(WavacityProject *project);
   ...
   /// Exports audio
   ///\param project Wavacity project
...
bool ExportFFmpeg::Init(const char *shortname, WavacityProject *project, const Tags *metadata, int subformat)
{
...
"FFmpeg : ERROR - Can't determine format description for file \"%s\".")
   ...
   // At the moment Wavacity can export only one audio stream
...
bool ExportFFmpeg::InitCodecs(WavacityProject *project)
{
   ...
   case FMT_OTHER:
      ...
      if (gPrefs->ReadBool(wxT("/FileFormats/FFmpegBitReservoir"),true))
         av_dict_set(&options, "reservoir", "1", 0);
      if (gPrefs->ReadBool(wxT("/FileFormats/FFmpegVariableBlockLen"),true)) mEncAudioCodecCtx->flags2 |= 0x0004; //WMA only?
...
//FIXME The list of supported options for the selected encoder should be extracted instead of a few hardcoded
   ...
   if (mEncAudioCodecCtx->codec_id == AV_CODEC_ID_AC3)
   {
      ...
      // But, currently, Wavacity only supports SAMPLE_FMT_S16.  So, for now, look for the
...
ProgressResult ExportFFmpeg::Export(WavacityProject *project,
   std::unique_ptr<ProgressDialog> &pDialog,
