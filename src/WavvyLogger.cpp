...
\class WavvyLogger
\brief WavvyLogger is a thread-safe logger class
...
#include "Wavvy.h" // This should always be included first
...
//     Wavvy may crash when terminating.  It's not fully understood why this occurs
...
// 2)  By providing an Wavvy specific logging class, it can be made thread-safe and,
//     as such, can be used by the ever growing threading within Wavvy.
//
enum
{
...
WavvyLogger *WavvyLogger::Get()
{
   ...
   std::call_once( flag, []{
      ...
      std::unique_ptr < wxLog > // DELETE any previous logger
         { wxLog::SetActiveTarget(safenew WavvyLogger) };
   } );

   // Use dynamic_cast so that we get a NULL ptr in case our logger
   // is no longer the target.
   return dynamic_cast<WavvyLogger *>(wxLog::GetActiveTarget());
}

WavvyLogger::WavvyLogger()
:  wxEvtHandler(),
   wxLog()
{
   mText = NULL;
...
void WavvyLogger::DoLogText(const wxString & str)
{
   if (!wxIsMainThread()) {
      wxMutexGuiEnter();
   }
   ...
   if (mBuffer.empty()) {
      ...
      mBuffer << stamp << _TS("Wavvy ") << WAVVY_VERSION_STRING << wxT("\n");
   }
...
bool WavvyLogger::SaveLog(const wxString &fileName) const
{
...
void WavvyLogger::Show(bool show)
{
   ...
   Destroy_ptr<wxFrame> frame
      { safenew wxFrame(NULL, wxID_ANY, _("Wavvy Log")) };
   ...
   frame->Bind(wxEVT_CLOSE_WINDOW,
                  wxCloseEventHandler(WavvyLogger::OnCloseWindow),
                  this);
...
wxString WavvyLogger::GetLog(int count)
{
   if (count == 0)
   {
      return mBuffer;
...
void WavvyLogger::OnCloseWindow(wxCloseEvent & WXUNUSED(e))
{
...
void WavvyLogger::OnClose(wxCommandEvent & WXUNUSED(e))
{
   wxCloseEvent dummy;
   OnCloseWindow(dummy);
}

void WavvyLogger::OnClear(wxCommandEvent & WXUNUSED(e))
{
   ClearLog();
}

void WavvyLogger::OnSave(wxCommandEvent & WXUNUSED(e))
{
