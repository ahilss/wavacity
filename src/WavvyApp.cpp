...
\class WavvyApp
\brief WavvyApp is the 'main' class for Wavvy

It handles initialization and termination by subclassing wxApp.

*//*******************************************************************/

#include "Wavvy.h" // This should always be included first; for USE_* macros and __UNIX__
#include "WavvyApp.h"
...
void PopulatePreferences()
{
   ...
   if (fn.FileExists())   // it will exist if the (win) installer put it there
   {
      ...
      auto pIni =
         WavvyFileConfig::Create({}, {}, fullPath, {},
...
"Reset Preferences?\n\nThis is a one-time question, after an 'install' where you asked to have the Preferences reset.");
   ...
   // In Wavvy 2.1.0 support for the legacy 1.2.x preferences (depreciated since Wavvy
   // 1.3.1) is dropped. As a result we can drop the import flag
   // first time this version of Wavvy is run we try to migrate
   // old preferences.
   ...
   // Check if some prefs updates need to happen based on wavvy version.
   ...
   // (meaning the changes won't mess wavvy up if the user goes back to an earlier version)
#ifndef __WXWASM__
...
static void QuitWavvy(bool bForce)
{
...
IMPLEMENT_APP_NO_MAIN(WavvyApp)
...
#else
IMPLEMENT_APP(WavvyApp)
#endif

#ifdef __WXMAC__

// in response of an open-document apple event
void WavvyApp::MacOpenFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a print-document apple event
void WavvyApp::MacPrintFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a open-application apple event
void WavvyApp::MacNewFile()
{
   if (!gInited)
      return;
...
#define kWavvyAppTimerID 0

BEGIN_EVENT_TABLE(WavvyApp, wxApp)
   EVT_QUERY_END_SESSION(WavvyApp::OnQueryEndSession)
   EVT_END_SESSION(WavvyApp::OnEndSession)

   EVT_TIMER(kWavvyAppTimerID, WavvyApp::OnTimer)
#ifdef __WXMAC__
   EVT_MENU(wxID_NEW, WavvyApp::OnMenuNew)
   EVT_MENU(wxID_OPEN, WavvyApp::OnMenuOpen)
   EVT_MENU(wxID_ABOUT, WavvyApp::OnMenuAbout)
   EVT_MENU(wxID_PREFERENCES, WavvyApp::OnMenuPreferences)
#endif
   ...
   // event
   EVT_MENU(wxID_EXIT, WavvyApp::OnMenuExit)

#if  !defined(__WXMSW__) && !defined(__WXWASM__)
   EVT_SOCKET(ID_IPC_SERVER, WavvyApp::OnServerEvent)
   EVT_SOCKET(ID_IPC_SOCKET, WavvyApp::OnSocketEvent)
#endif

   // Recent file event handlers.
   EVT_MENU(FileHistory::ID_RECENT_CLEAR, WavvyApp::OnMRUClear)
   EVT_MENU_RANGE(FileHistory::ID_RECENT_FIRST, FileHistory::ID_RECENT_LAST,
      WavvyApp::OnMRUFile)

   // Handle AppCommandEvents (usually from a script)
   EVT_APP_COMMAND(wxID_ANY, WavvyApp::OnReceiveCommand)

   // Global ESC key handling
   EVT_KEY_DOWN(WavvyApp::OnKeyDown)
END_EVENT_TABLE()
...
bool WavvyApp::MRUOpen(const FilePath &fullPathStr) {
   // Most of the checks below are copied from ProjectManager::OpenFiles.
   // - some rationalisation might be possible.

   WavvyProject *proj = GetActiveProject();
   ...
   if (!fullPathStr.empty())
   {
      ...
      if (wxFile::Exists(fullPathStr))
      {
         ...
         // Test here even though WavvyProject::OpenFile() also now checks, because
...
bool WavvyApp::SafeMRUOpen(const wxString &fullPathStr)
{
   return GuardedCall< bool >( [&]{ return MRUOpen( fullPathStr ); } );
}

void WavvyApp::OnMRUClear(wxCommandEvent& WXUNUSED(event))
{
...
void WavvyApp::OnTimer(wxTimerEvent& WXUNUSED(event))
{
  WavvyProject *proj = GetActiveProject();
  try {
  ...
  return;
   ...
   // Filenames are queued when Wavvy receives a few of the
...
void WavvyApp::OnFatalException()
{
#if defined(EXPERIMENTAL_CRASH_REPORT)
...
bool WavvyApp::OnExceptionInMainLoop()
{
   return true;
...
bool WavvyApp::OnInit()
{
   ...
   if ( !ProjectFileIO::InitializeSQL() )
      this->CallAfter([]{
         ::WavvyMessageBox(
            XO("SQLite library failed to initialize.  Wavvy cannot continue.") );
         QuitWavvy( true );
      });


   // cause initialization of wxWidgets' global logger target
   (void) WavvyLogger::Get();
   ...
   gtk_rc_parse_string("style \"wavvy\" {\n"
                       " GtkButton::inner_border = { 0, 0, 0, 0 }\n"
                       " GtkEntry::inner_border = { 0, 0, 0, 0 }\n"
                       " xthickness = 4\n"
                       " ythickness = 0\n"
                       "}\n"
                       "widget_class \"*GtkCombo*\" style \"wavvy\"");
#endif

   // Don't use WAVVY_NAME here.
   // We want Wavvy with a capital 'A'

// DA: App name
#ifndef EXPERIMENTAL_DA
   wxString appName = wxT("Wavvy");
#else
   wxString appName = wxT("DarkWavvy");
#endif
   ...
   //
   FilePaths wavvyPathList;

#ifdef __WXGTK__
   /* Search path (for plug-ins, translations etc) is (in this order):
      * The WAVVY_PATH environment variable
      * The current directory
      * The user's "~/.wavvy-data" or "Portable Settings" directory
      * The user's "~/.wavvy-files" directory
   ...
   if (!envTempDir.empty()) {
      ...
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("%s/wavvy-%s"), envTempDir, wxGetUserId() ) );
   } else {
      /* On Unix systems, the default temp dir is in /var/tmp. */
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("/var/tmp/wavvy-%s"), wxGetUserId() ) );
   }

// DA: Path env variable.
#ifndef EXPERIMENTAL_DA
   wxString pathVar = wxGetenv(wxT("WAVVY_PATH"));
#else
   wxString pathVar = wxGetenv(wxT("DARKWAVVY_PATH"));
#endif
   if (!pathVar.empty())
      FileNames::AddMultiPathsToPathList(pathVar, wavvyPathList);
   FileNames::AddUniquePathToPathList(::wxGetCwd(), wavvyPathList);

   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, wavvyPathList);

   FileNames::AddUniquePathToPathList(FileNames::DataDir(), wavvyPathList);
   ...
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/share/%s"),
      wxT(INSTALL_PREFIX), wxT(WAVVY_NAME)),
      wavvyPathList);
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/share/doc/%s"),
      wxT(INSTALL_PREFIX), wxT(WAVVY_NAME)),
      wavvyPathList);
#else //WAVVY_NAME
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/.wavvy-files"),
      home),
      wavvyPathList)
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/share/wavvy"),
      wxT(INSTALL_PREFIX)),
      wavvyPathList);
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/share/doc/wavvy"),
      wxT(INSTALL_PREFIX)),
      wavvyPathList);
#endif //WAVVY_NAME
...
// JKC Bug 1220: Use path based on home directory on WXMAC
   ...
   // On Mac and Windows systems, use the directory which contains Wavvy.
#ifdef __WXMSW__
   // On Windows, the path to the Wavvy program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, wavvyPathList);
   FileNames::AddUniquePathToPathList(progPath + wxT("\\Languages"), wavvyPathList);
...
#ifdef __WXMAC__
   // On Mac OS X, the path to the Wavvy program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);

   FileNames::AddUniquePathToPathList(progPath, wavvyPathList);
   // If Wavvy is a "bundle" package, then the root directory is
   // the great-great-grandparent of the directory containing the executable.
   //FileNames::AddUniquePathToPathList(progPath + wxT("/../../../"), wavvyPathList);

   // These allow for searching the "bundle"
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../"), wavvyPathList);
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../Resources"), wavvyPathList);
   ...
   TempDirectory::SetDefaultTempDir( wxString::Format(
      wxT("%s/Library/Application Support/wavvy/SessionData"), tmpDirLoc) );
...
#ifdef __WXWASM__
   FileNames::AddUniquePathToPathList(wxT("locale"), wavvyPathList);
   FileNames::AddUniquePathToPathList(wxT("share/wavvy"), wavvyPathList);
#endif

   FileNames::SetWavvyPathList( std::move( wavvyPathList ) );
   ...
   {
      wxFileName configFileName(FileNames::DataDir(), wxT("wavvy.cfg"));
   ...
   // Wavvy is running, we must return from OnInit() to wxWidgets before
...
bool WavvyApp::InitPart2()
{
   ...
   wxImage logoimage((const char **)WavvyLogoWithName_xpm);
   ...
   WavvyProject *project;
   {
      ...
      // as where Wavvy project will appear.
      wxRect wndRect;
      ...
      temporarywindow.SetTitle(_("Wavvy is starting up..."));
      ...
      fileMenu->Append(wxID_ABOUT, _("&About Wavvy..."));
   ...
   // Workaround Bug 1377 - Crash after Wavvy starts and low disk space warning appears
   ...
   CallAfter( [=] () mutable {
      ...
      int vMajorInit = WAVVY_VERSION, vMinorInit = WAVVY_RELEASE, vMicroInit = WAVVY_REVISION;
#ifndef __WXWASM__
      gPrefs->GetVersionKeysInit(vMajorInit, vMinorInit, vMicroInit);
#endif
      if (vMajorInit != WAVVY_VERSION || vMinorInit != WAVVY_RELEASE
         || vMicroInit != WAVVY_REVISION) {
   ...
   mTimer.SetOwner(this, kWavvyAppTimerID);
...
#if defined(__WXMAC__)
   // The first time this version of Wavvy is run or when the preferences
   // are reset, execute the "tccutil" command to reset the microphone permissions
   // currently assigned to Wavvy.  The end result is that the user will be
   // prompted to approve/deny Wavvy access (again).
   //
   // This should resolve confusion of why Wavvy appears to record, but only
   // gets silence due to Wavvy being denied microphone access previously.
   bool permsReset = false;
   gPrefs->Read(wxT("/MicrophonePermissionsReset"), &permsReset, false);
   if (!permsReset) {
      system("tccutil reset Microphone org.audacityteam.wavvy");
...
void WavvyApp::InitCommandHandler()
{
...
void WavvyApp::OnReceiveCommand(AppCommandEvent &event)
{
   wxASSERT(NULL != mCmdHandler);
   mCmdHandler->OnReceiveCommand(event);
}

void WavvyApp::OnKeyDown(wxKeyEvent &event)
{
...
bool WavvyApp::InitTempDir()
{
   ...
   if (temp.empty()) {
      ...
      if( !TempDirectory::IsTempDirectoryNameOK( tempFromPrefs ) ) {
         WavvyMessageBox(XO(
"Wavvy could not find a safe place to store temporary files.\nWavvy needs a place where automatic cleanup programs won't delete the temporary files.\nPlease enter an appropriate directory in the preferences dialog."));
      } else {
         WavvyMessageBox(XO(
"Wavvy could not find a place to store temporary files.\nPlease enter an appropriate directory in the preferences dialog."));
      }
      ...
      WavvyMessageBox(XO(
"Wavvy is now going to exit. Please launch Wavvy again to use the new temporary directory."));
      return false;
   }
   #endif
...
// Return true if there are no other instances of Wavvy running,
// false otherwise.

bool WavvyApp::CreateSingleInstanceChecker(const wxString &dir)
{
   wxString name = wxString::Format(wxT("wavvy-lock-%s"), wxGetUserId());
   mChecker.reset();
   auto checker = std::make_unique<wxSingleInstanceChecker>();

   auto runningTwoCopiesStr = XO("Running two copies of Wavvy simultaneously may cause\ndata loss or cause your system to crash.\n\n");
   ...
   if (!checker->Create(name, dir))
   {
      ...
      auto prompt = XO(
"Wavvy was not able to lock the temporary files directory.\nThis folder may be in use by another copy of Wavvy.\n")
         + runningTwoCopiesStr
         + XO("Do you still want to start Wavvy?");
      int action = WavvyMessageBox(
         prompt,
      ...
      // to an already active Wavvy.  If successful, we send
      // the first command line argument (the audio file name)
      // to that Wavvy for processing.
      wxClient client;
      ...
      for (int i = 0; i < 50; i++)
      {
         ...
         if (conn)
         {
            ...
            {
               // Send an empty string to force existing Wavvy to front
      ...
      // There is another copy of Wavvy running.  Force quit.

      auto prompt =  XO(
"The system has detected that another copy of Wavvy is running.\n")
         + runningTwoCopiesStr
         + XO(
"Use the New or Open commands in the currently running Wavvy\nprocess to open multiple projects simultaneously.\n");
      WavvyMessageBox(
         prompt, XO("Wavvy is already running"),
         wxOK | wxICON_ERROR);
...
void WavvyApp::OnServerEvent(wxSocketEvent & /* evt */)
{
}

void WavvyApp::OnSocketEvent(wxSocketEvent & /* evt */)
{
}
...
bool WavvyApp::CreateSingleInstanceChecker(const wxString &dir)
{
   ...
   // time Wavvy has been run during this boot of the system. In this
   ...
   if (isServer)
   {
      ...
      {
         WavvyMessageBox(
            XO("The Wavvy IPC server failed to initialize.\n\n"
               "This is likely due to a resource shortage\n"
               "and a reboot may be required."),
            XO("Wavvy Startup Failure"),
            wxOK | wxICON_ERROR);
   ...
   // If we get here, then Wavvy is currently active. So, we connect
   ...
   // Attempt to connect to an active Wavvy.
   ...
   // Display Wavvy's version if requested
   if (parser->Found(wxT("v")))
   {
      wxPrintf("Wavvy v%s\n", WAVVY_VERSION_STRING);
   ...
   // go to WavvyApp::MacOpenFile. Forward the file names to the prior
...
std::unique_ptr<wxCmdLineParser> WavvyApp::ParseCommandLine()
{
   ...
   /*i18n-hint: This controls the number of bytes that Wavvy will
   ...
   /*i18n-hint: This runs a set of automatic tests on Wavvy itself */
   parser->AddSwitch(wxT("t"), wxT("test"), _("run self diagnostics"));

   /*i18n-hint: This displays the Wavvy version */
   parser->AddSwitch(wxT("v"), wxT("version"), _("display Wavvy version"));

   /*i18n-hint: This is a list of one or more files that Wavvy
...
void WavvyApp::OnQueryEndSession(wxCloseEvent & event)
{
   bool mustVeto = false;
...
void WavvyApp::OnEndSession(wxCloseEvent & event)
{
...
void WavvyApp::OnMenuAbout(wxCommandEvent & /*event*/)
{
...
void WavvyApp::OnMenuNew(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
...
void WavvyApp::OnMenuOpen(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void WavvyApp::OnMenuPreferences(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void WavvyApp::OnMenuExit(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
//BG: On Windows, associate the aup file type with Wavvy
...
*/
#if defined(__WXMSW__) && !defined(__WXUNIVERSAL__) && !defined(__CYGWIN__)
void WavvyApp::AssociateFileTypes()
{
   ...
   auto DefineType = [&](const wxString &type)
   {
      ...
      if (!root_key.empty())
      {
         associateFileTypes = wxT("Wavvy.Project"); // Finally set value for the key
      }
   ...
   if (IsDefined(wxT(".aup3")) && IsDefined(wxT(".aup")) && IsDefined(wxT("Wavvy.Project")))
   {
   ...
   int wantAssoc =
      WavvyMessageBox(
         XO(
"Wavvy project (.aup3) files are not currently \nassociated with Wavvy. \n\nAssociate them, so they open on double-click?"),
         XO("Wavvy Project Files"),
   ...
   else
   {
      ...
      associateFileTypes = wxT("Wavvy.Project"); // Finally set value for .AUP key
      associateFileTypes.SetName(root_key + wxT("Wavvy.Project"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
         associateFileTypes = wxT("Wavvy Project File");
      }

      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell\\open"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      }

      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell\\open\\command"));
      ...
      if (!associateFileTypes.Exists() ||
            (tmpRegAudPath.Find(wxT("wavvy.exe")) >= 0))
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell\\open\\ddeexec"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell\\open\\ddeexec\\Application"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("Wavvy.Project\\shell\\open\\ddeexec\\Topic"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
