...
void SpectrogramSettings::LoadPrefs()
{
   ...
   isGrayscale = gPrefs->ReadBool(wxT("/Spectrum/Grayscale"), false);

   scaleType = ScaleType(gPrefs->Read(wxT("/Spectrum/ScaleType"), 0L));

#ifndef SPECTRAL_SELECTION_GLOBAL_SWITCH
   spectralSelection = gPrefs->ReadBool(wxT("/Spectrum/EnableSpectralSelection"), true);
#endif

   algorithm = Algorithm(gPrefs->Read(wxT("/Spectrum/Algorithm"), 0L));

#ifdef EXPERIMENTAL_FFT_Y_GRID
   fftYGrid = gPrefs->ReadBool(wxT("/Spectrum/FFTYGrid"), false);
#endif //EXPERIMENTAL_FFT_Y_GRID

#ifdef EXPERIMENTAL_FIND_NOTES
   fftFindNotes = gPrefs->ReadBool(wxT("/Spectrum/FFTFindNotes"), false);
   findNotesMinA = gPrefs->Read(wxT("/Spectrum/FindNotesMinA"), -30.0);
   numberOfMaxima = gPrefs->Read(wxT("/Spectrum/FindNotesN"), 5L);
   findNotesQuantize = gPrefs->ReadBool(wxT("/Spectrum/FindNotesQuantize"), false);
