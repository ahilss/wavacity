...
void FileNames::SelectFile(Operation op,
   const TranslatableString& message,
   const FilePath& default_path,
   const FilePath& default_filename,
   const FileExtension& default_extension,
   ...
   int flags,
   wxWindow *parent,
   std::function<void (const FilePath&)> callback)
{
    wxString filter;
    if ( !default_extension.empty() )
       filter = wxT("*.") + default_extension;
    auto path = gPrefs->Read(PreferenceKey(op, PathType::User), default_path);
    if (path.empty())
    ...
    FileSelector(
          ...
          FormatWildcard( fileTypes ),
          flags, parent, wxDefaultCoord, wxDefaultCoord,
          [op, callback](const FilePath& result) {
