...
WavacityFileConfig::WavacityFileConfig(
...
WavacityFileConfig::~WavacityFileConfig() = default;

std::unique_ptr<WavacityFileConfig> WavacityFileConfig::Create(
   const wxString& appName,
   const wxString& vendorName,
   const wxString& localFilename,
   const wxString& globalFilename,
...
)
{
   ...
   auto result = std::unique_ptr<WavacityFileConfig>{
      safenew WavacityFileConfig{
         appName, vendorName, localFilename, globalFilename, style, conv } };
   result->Init();
   return result;
}

void WavacityFileConfig::Warn()
{
   wxDialogWrapper dlg(nullptr, wxID_ANY, XO("Wavacity Configuration Error"));
   ...
   S.StartVerticalLay(wxEXPAND, 1);
   {
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         S.AddFixedText(
            XO("The following configuration file could not be accessed:\n\n"
               ...
               "If you choose to \"Quit Wavacity\", your project may be left in an unsaved "
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         ...
         b = S.Id(wxID_CANCEL).AddButton(XXO("&Quit Wavacity"));
   ...
   switch (dlg.ShowModal())
   {
      case wxID_HELP:
         ...
         OpenInDefaultBrowser("https://" +
                              ...
                              "Error:_Wavacity_settings_file_unwritable");
      break;
