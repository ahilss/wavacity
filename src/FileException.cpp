...
TranslatableString FileException::ErrorMessage() const
{
   ...
   switch (cause) {
      case Cause::Open:
         format = XO("Wavacity failed to open a file in %s.");
         break;
      case Cause::Read:
         format = XO("Wavacity failed to read from a file in %s.");
         break;
      case Cause::Write:
         return WriteFailureMessage(fileName);
      case Cause::Rename:
         format =
XO("Wavacity successfully wrote a file in %s but failed to rename it as %s.");
      default:
         break;
   }
...
{
   return XO("Wavacity failed to write to a file.\n"
