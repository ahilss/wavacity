...
TranslatableString FileException::ErrorMessage() const
{
   ...
   switch (cause) {
      case Cause::Open:
         format = XO("Wavvy failed to open a file in %s.");
         break;
      case Cause::Read:
         format = XO("Wavvy failed to read from a file in %s.");
         break;
      case Cause::Write:
         return WriteFailureMessage(fileName);
      case Cause::Rename:
         format =
XO("Wavvy successfully wrote a file in %s but failed to rename it as %s.");
      default:
         break;
   }
...
{
   return XO("Wavvy failed to write to a file.\n"
