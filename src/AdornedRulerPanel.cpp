...
AdornedRulerPanel::QuickPlayIndicatorOverlay::DoGetRectangle(wxSize size)
{
   ...
   // TODO(ahilss): fix update region
   wxRect rect(mOldQPIndicatorPos - 1, 0, 2, size.GetHeight());
...
, mProject(project)
{
   ...
   mTimelineToolTip = gPrefs->ReadBool(wxT("/QuickPlay/ToolTips"), true);
   mPlayRegionDragsSelection = gPrefs->ReadBool(wxT("/QuickPlay/DragSelection"), false);
   mQuickPlayEnabled = gPrefs->ReadBool(wxT("/QuickPlay/QuickPlayEnabled"), true);
...
void AdornedRulerPanel::ShowMenu(const wxPoint & pos)
{
   ...
   const auto &playRegion = viewInfo.playRegion;
   wxMenu *rulerMenu = safenew wxMenu();
...
void AdornedRulerPanel::ShowScrubMenu(const wxPoint & pos)
{
   ...
   PopupMenu(rulerMenu, pos, [this, rulerMenu] (bool retCode) {
      PopEventHandler();
   });
}
