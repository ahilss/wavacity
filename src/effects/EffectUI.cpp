...
void EffectUIHost::ShowModal(std::function<void (int)> callback)
{
   ...
   return wxDialogWrapper::ShowModal(callback);
}
...
wxBitmap EffectUIHost::CreateBitmap(const char * const xpm[], bool up, bool pusher)
{
   ...
   wxBitmap mod;
   mod.CreateScaled(pic.GetWidth() + 6, pic.GetHeight() + 6, 24, GetContentScaleFactor());
   dc.SelectObject(mod);
