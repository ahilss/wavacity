...
void Effect::ShowInterfaceAsync(wxWindow &parent,
   ...
   std::function<void (bool)> callback)
{
   ...
   if (mClient)
   {
      mClient->ShowInterfaceAsync(parent, factory, forceModal, callback);
      return;
   }
...
bool Effect::MaybeProcess(NotifyingSelectedRegion &selectedRegion)
{
   bool returnVal = true;
                      ...
                      const EffectDialogFactory &dialogFactory)
{
   ...
   if ( pParent && dialogFactory &&
        IsInteractive() )
   {
      ShowInterfaceAsync( *pParent, dialogFactory, IsBatchProcessing(),
         [this, &selectedRegion](bool result)
      {
         if (result)
         {
            this->MaybeProcess(selectedRegion);
         }
      });
   }
   else
   {
      success = MaybeProcess(selectedRegion);
      return success;
   }
